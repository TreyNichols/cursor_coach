...
import pygame
import mouse
from constants import (WIDTH, HEIGHT, CURSOR_SIZE)
...
class My_Cursor():
    ...
    def update(self):
        ...
        if self.x > WIDTH - (CURSOR_SIZE // 2):
            self.x = WIDTH - (CURSOR_SIZE // 2)
        if self.y < 0 - (CURSOR_SIZE // 2):
            self.y = 0 - (CURSOR_SIZE // 2)
        if self.y >= HEIGHT - (CURSOR_SIZE // 2):
            self.y = HEIGHT - (CURSOR_SIZE // 2)
        
        self.rect.x = self.x + CURSOR_SIZE//2 - 1
        self.rect.y = self.y + CURSOR_SIZE//2 - 1    
        
        # Moves mouse if it hits the edge
        if (pygame.mouse.get_pos()[0] == 0 or pygame.mouse.get_pos()[0] >= WIDTH-1 or 
                pygame.mouse.get_pos()[1] == 0 or pygame.mouse.get_pos()[1] >= HEIGHT-1):
